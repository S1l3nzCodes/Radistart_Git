...
if str("becker-f") or str("flo") in User:
    print "Benutzer ist Florian"
else:
    exit()
...
def PHP():
    if str("Flo") in User:
        webbrowser.open_new("http://192.168.2.200/phpmyadmin")
    else:
        webbrowser.open_new("http://172.24.105.118/phpmyadmin")

def Server():
    if str("Flo") in User:
        webbrowser.open_new("http://192.168.2.200")
    else:
        webbrowser.open_new("http://172.24.105.118")
...
def wunschfenster():
    ...
    global eWunsch
    BB=tk.Button(fWunsch, text="Verbesserungswunsch abschicken", font=buttonfont, command=error, bg=grau)
    BB.grid(row=1, column=3)
...
#GUI Layout
main=tk.Tk()
main.title('Radistart Administratorversion')
main.geometry('1200x700+20+20')
Hauptframe=tk.Frame(main, bd=1)
Hauptframe.pack(side="top",expand=1,fill="both")

#Spalten
Spalte_links=tk.Frame(Hauptframe, bd=2,bg=config.dunkelgrau)
Spalte_links.pack(side="left",expand=1, fill="both")
Spalte_mitte=tk.Frame(Hauptframe, bd=2, bg=config.dunkelgrau)
Spalte_mitte.pack(side="left", expand=1, fill="both")
Spalte_rechts=tk.Frame(Hauptframe, bd=2, bg=config.dunkelgrau)
...
Spalte_info=tk.Frame(Spalte_rechts, bg=config.grau)
Spalte_info.pack(side="bottom",expand=1, fill="both")

Frame_telefon=tk.Frame(Spalte_mitte, bg=config.grau, bd=2, relief="raised")
Frame_telefon.pack(expand=1, fill="both")

Frame_telefon_Buttons=tk.Frame(Frame_telefon, bg=config.dunkelgrau)
...
Button_intranet=tk.Button(halberbutton, text="Intranet UKSH", font=config.buttonfont, bg=config.grau,bd=4, command=intranet)
Button_intranet.pack(expand=1,fill="both")

Button_Rad=tk.Button(halberbutton, text="Intranet Radiologie", font=config.buttonfont, bg=config.grau,bd=4, command=intranet_Radiologie)
Button_Rad.pack(expand=1, fill="both")

Button_qm=tk.Button(Spalte_links, text="roXtra - QM", font=config.buttonfont, bg=config.grau,bd=4, command=qm)
Button_qm.pack(expand=1, fill="both")

Button_spx=tk.Button(Spalte_links, text="SP-Expert", font=config.buttonfont, bg=config.grau,bd=4, command=spx)
Button_spx.pack(expand=1, fill="both")

Button_logbuch=tk.Button(halberbutton2, text="Logbuch",font=config.buttonfont, bg=config.grau,bd=4, command=transport)
Button_logbuch.pack(expand=1,fill="both")

Button_server=tk.Button(Spalte_links, text="Server", font=config.buttonfont, bg=config.grau,bd=4, command=Server)
Button_server.pack(expand=1, fill="both")

Button_PHP=tk.Button(Spalte_links, text="PHPmyAdmin", font=config.buttonfont, bg=config.grau,bd=4, command=PHP)
Button_PHP.pack(expand=1, fill="both")

Button_speiseplan=tk.Button(Spalte_links, text="Speiseplan", font=config.buttonfont, bg=config.grau,bd=4, command=essen)
Button_speiseplan.pack(expand=1, fill="both")


#Buttons rechts
Button_Word=tk.Button(Spalte_rechts, text="Microsoft Word", font=config.buttonfont, bg=config.grau,bd=4, command=word)
Button_Word.pack(expand=1, fill="both")

Button_Excel=tk.Button(Spalte_rechts, text="Microsoft Excel", font=config.buttonfont, bg=config.grau,bd=4, command=excel)
Button_Excel.pack(expand=1, fill="both")

Button_Outlook=tk.Button(Spalte_rechts, text="Email / Webmail", font=config.buttonfont, bg=config.grau,bd=4, command=outlook)
Button_Outlook.pack(expand=1, fill="both")

Button_Dokumente=tk.Button(Spalte_rechts, text="Dokumente", font=config.buttonfont, bg=config.grau, bd=4, command=doks)
Button_Dokumente.pack(expand=1, fill="both")

Button_Bestellung=tk.Button(Spalte_rechts, text="Bestellwünsche", font=config.buttonfont, bg=config.grau,bd=4, command=error)
Button_Bestellung.pack(expand=1, fill="both")

Button_Besprechung=tk.Button(Spalte_rechts, text="Besprechungswunsch", bg=config.grau, font=config.buttonfarbe,bd=4, command=error)
Button_Besprechung.pack(expand=1, fill="both")

Button_Orbis=tk.Button(Spalte_rechts, text="Orbis", bg=config.grau ,font=config.buttonfarbe,bd=4, command=orbis)
...
Label_datum=tk.Label(Spalte_info, text=Datum, font=config.schriftmittel, bg=config.grau)
Label_datum.pack(expand=1, fill="both")
    
Label_nutzer=tk.Label(Spalte_info, text="Benutzer: "+User+"\n"+"Rechnername: "+Host, font=config.buttonfont, bg=config.grau)
Label_nutzer.pack(expand=1, fill="both")

Label_Abteilung=tk.Label(Spalte_info, text=Abteilung, font=config.buttonfont, bg=config.grau)
...
Label_Radiologietelefonbuch=tk.Label(Frame_telefon, font=config.schriftgross, text="Telefonbuch für die Radiologie", bg=config.grau)
Label_Radiologietelefonbuch.pack(expand=1, fill="x")
Frame_telefon2=tk.Frame(Frame_telefon,bd=4, bg=config.grau)
Frame_telefon2.pack(expand=1, fill="x")
Label_Suche=tk.Label(Frame_telefon2, font=config.buttonfont, text="Suchen: ", bg=config.grau)
...
Button_Radiologietelefonbuch=tk.Button(Frame_telefon_Buttons, font=config.buttonfont, text="Nummer suchen", command=telefon_suche, bg=config.grau)
Button_Radiologietelefonbuch.pack(side="left",expand=1, fill="both")
Button_Radilogietelefonbuch_Neu=tk.Button(Frame_telefon_Buttons, font=config.buttonfont, text="Neue Suche", command=NeueSuche, bg=config.grau)
...
#EMAIL PLatzhalter
Email_platzhalter=tk.Label(Spalte_email, font=config.buttonfont,height=20,width=60, bg=config.grau, text="Platzhalter...")
